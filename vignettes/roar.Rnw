...
\author{Elena Grassi$^{1}$, Antonio Lembo$^{1}$, Elisa Mariella$^{1}$, Ivan Molineris$^{1}$ 
\\ and Paolo Provero$^{1,2}$ \\
  $^1$ Department of Molecular Biotechnologies and Health Sciences,\\ MBC, University of Turin, Italy\\
...
\date{\Rpackage{roar} version \Sexpr{packageDescription("roar")$Version} (Last revision 2013-10-14)}
\begin{document}
...
\textbf{XXX Paolo told me that it's the opposite}
Therefore
...
splicing events.
\textbf{XXX For Paolo it's not clear which features are peculiar to example and which are needed}
...
(up until 2.5Gb, ~58.5 million reads) 4 Gb of RAM will be more than sufficient (peak memory 
...
For bigger datasets (eg. 20Gb, ~413 million reads) you will have to split your alignments in 
small chunks: \emph{roarWrapper\_chrBychr} works on single chromosomes.
\textbf{Discuss read length?}
...
# HNRNPC ko data
bamRight <- RNAseqData.HNRNPC.bam.chr14_BAMFILES[seq(5,8)]
# control (HeLa wt)
bamLeft <- RNAseqData.HNRNPC.bam.chr14_BAMFILES[seq(1,3)]
...
\textbf{XXX Paolo wonders if using total counts could be better to consider library size}
%<<eval=FALSE>>=
<<>>= 
...
possible combinations p-values for multiple samples analysis. \textbf{XXX Fisher procedure!}

In this case for example there will be columns ``pvalue\_1\_1'' up to ``pvalue\_4\_3'': the first
number represents the number of the considered sample for the right condition, while
...
\textbf{XXX right should be treatment and left control}
...
smaller than the given cutoff. It is worth noting that all this p-values are nominal and multiple
...
\textbf{XXX Paolo: equiprobability only later, k in denominator, remove
system solution}
...
therefore assuming that each nucleotide has the same probability of giving rise to a read (equiprobability
assumption) we
can say that the total number of reads falling over a transcript in its entirety ($N$) derives
%and we know that the total number of reads falling over a transcript ($N$) in its entirety derives
...
\textbf{[ $N_{PRE} / N_{TOT} \doteq l_{PRE}$ and similarly $N_{POST} / N_{TOT} \doteq l_{POST}$
assuming that $N_{PRE}$ and $N_{POST}$ are XXX. TODO which is the definition of $N_{PRE}$? Difference
from $\#r_{PRE}$, \ldots. ?
Do we need this equation to explain the following part? It does not seem so...
We are now considering reads across PRE/POST, does this assumption difference change something?]}
...
Assuming the equiprobability of read distribution (that is each nucleotide has the same probability 
...
\textbf{Please note XXX due parole sul fatto che consideriamo sia qui che sopra la lunghezza ma 
...
\begin{math} \left\{
  \begin{array}{l}
      m = \frac{\#r_{PRE}-\epsilon_{M} \frac{\#r_{POST}}{\epsilon_{M} (\frac{l_{POST}}{l_{PRE}+l_{POST}}) }
      (\frac{l_{PRE}}{l_{PRE}+l_{POST}})}{\epsilon_{m} } \\
      M = ``
  \end{array} \right.
\end{math}
\vspace{0.5cm} \\
\begin{math} \left\{
  \begin{array}{l}
      m = \frac{\#r_{PRE}-  \#r_{POST} (\frac{l_{PRE}+l_{POST}}{l_{POST}}) (\frac{l_{PRE}}{l_{PRE}+l_{POST}})}{\epsilon_{m} } \\
      M = ``
  \end{array} \right.
\end{math}
\vspace{0.5cm} \\
\begin{math} \left\{
  \begin{array}{l}
      m = \frac{\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}})}{\epsilon_{m} } \\
      M = ``
  \end{array} \right.
\end{math}
\vspace{0.5cm} \\
\begin{math} \left\{
  \begin{array}{l}
      m = (\frac{k}{l_{PRE}}) (\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}})) \\
      M = \frac{k}{l_{PRE}+l_{POST}} \frac{(l_{PRE}+l_{POST})}{l_{POST}} \#r_{POST}
  \end{array} \right.
\end{math}
\vspace{0.5cm} \\
\begin{math}
m/M = \frac{(\frac{k}{l_{PRE}}) (\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}}))}{ (\frac{k}{l_{POST}}) \#r_{POST}}
\end{math}
\vspace{0.5cm} \\
\begin{math}
m/M =  \frac{l_{POST}}{l_{PRE} \#r_{POST}} (\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}})) 
\end{math}
\vspace{0.5cm} \\
\begin{math}
m/M = \frac{l_{POST} \#r_{PRE}}{l_{PRE} \#r_{POST}} 
- \frac{l_{POST} \#r_{POST} l_{PRE}}{\#r_{POST} l_{PRE} l_{POST}}
\end{math}
\vspace{0.5cm} \\
And finally the formula to obtain $m/M$ using only read counts and lengths:\vspace{0.5cm}\\
\begin{math}
...
\begin{math}
l^{\prime}_{PRE} = l_{PRE} % come si scrivono gli apici in math mode?
\end{math}
\vspace{0.2cm} \\
\begin{math}
l^{\prime}_{POST} = l_{POST} + readLength - 1
\end{math}
\vspace{0.5cm} \\ % find a suitable relative measure XXX TODO
