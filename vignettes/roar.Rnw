...
\textbf{Get some sentences from the poster abstract? XXX}
...
at the beginning of the last exon (or at the APA site if it falls on the last exon) and ends
...
via the ps command with the keyword rss, when comparing two samples of 2.5Gb: ~1.3Gb) and you can use the 
\emph{roarWrapper}  script.
For bigger datasets (eg. 20Gb, ~413 million reads) you will have to split your alignments in small chunks: 
...
a condition every combination of comparisons between the samples lists is considered - for example
in this case we have four samples for each condition, therefore it will calculate 16 p-values.
...
possible combinations p-values for multiple samples analysis. 
