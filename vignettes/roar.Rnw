...
upon the hg19 human genome release using the PolyA\_DB (\cite{XXX}) track to obtain
...
with \emph{countResults} and knit together real FPKM values afterwards, if you need them.
