...
\begin{abstract}
  ...
  The method is described in \cite{XXX}. TODO
...
\section{Input data}
A roar analysis start from some alignments, obtained via standard RNAseq analyses.
It is possible to build a \Rclass{RoarDataset} object giving its constructor
two lists of bam files representing the two conditions to be compared or to
...
upon the hg19 human genome release using the PolyAdb (\cite{XXX}) database for 
coordinates of alternative polyadenylation sites (from now on addressed as APA).
Every gene with at least an APA site is considered; 
its longest transcript
is used to get the ``classical'' polyadenylation syte (canonical end of the longest transcript stored
in UCSC) and the more distal (with respect to the TSS XXX TODO) reported APA
site found in PolyAdb is used to define the end of the short isoform. Therefore
...
(up until 5Gb, TODO CHECK) XXX Gb will suffice and you can use the \emph{roarWrapper} script.
Otherwise you will have to split your alignments in small chunks: \emph{roarWrapper\_chrBychr}
works on single chromosomes. 

Please note than when working in this way FPKM
values returned by \emph{fpkmResults} will be different from those deriving from the analysis
performed with a single \Rclass{RoarDataset} on the whole alignments - in this case you can extract counts 
with \emph{countResults} and knit together real FPKM afterwards, if you need them.
...
the alignment data and the annotation regarding 3'UTRs coordinates.
<<eval=FALSE>>=
library(roar)
library(RNAseqData.HNRNPC.bam.chr14)
gtf <- system.file("examples", "apa.gtf", package="roar")
bamRight <- RNAseqData.HNRNPC.bam.chr14_BAMFILES[seq(5,8)] # HNRNPC ko data
bamLeft <- RNAseqData.HNRNPC.bam.chr14_BAMFILES[seq(1,4)]  # control (HeLa wt)
...
Reads of the given bam annotation files that falls over this portion 
are accounted for with the following rules:
\begin{enumerate}
       ...
       \item reads falling over a single gene coordinates but both on PRE and POST are assigned 
...
case we use FALSE as long as we don't want to consider strand information.
...
over the other one.
For details about the m/M calculations see XXX.
...
If for one of the conditions there are more than one samples, like in this example
that has four samples for each condition, then calculations are performed on average counts.
<<eval=FALSE>>= 
...
There are various functions aimed at extracting analyses results from a \Rclass{RoarDataset};
...
represents the roar and it's bigger than one when the shorter isoform is more expressed in the right
...
In this case for example there will be columns ``pvalue\_1\_1'' up to ``pvalue\_4\_4'': the first
number represent the number of the considered sample for the right condition, while
the second for the left one (the order given in the list given in the RoarDataset constructors
is kept).
...
nrow(results_filtered[results_filtered$nUnderCutoff==16,])
@
Note that for FPKM we used -Inf as a cutoff because in this case we didn't want to filter out
genes considering their expression values (and because we used only a partial alignments
