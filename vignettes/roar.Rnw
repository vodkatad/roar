...
\date{\Rpackage{roar} version \Sexpr{packageDescription("roar")$Version} (Last revision 2013-10-10)}
\begin{document}
...
assuming that $N_{PRE}$ and $N_{POST}$ are XXX. TODO which is the definition of N? Difference
from $\#r_{PRE}$, \ldots. ?
...
\begin{math} \left\{
  \begin{array}{l}
      m = k/l_{PRE} (\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}})) \\
      M = \frac{k}{l_{PRE}+l_{POST}} \frac{(l_{PRE}+l_{POST})}{l_{POST}} \#r_{POST}
  \end{array} \right.
\end{math}
\vspace{0.5cm} \\
\begin{math}
m/M = \frac{k/l_{PRE} (\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}}))}{k/l_{POST} \#r_{POST}}
\end{math}
\vspace{0.5cm} \\
\begin{math}
m/M =  \frac{l_{POST}}{\#r_{PRE} l_{PRE}} (\#r_{PRE}-\#r_{POST} (\frac{l_{PRE}}{l_{POST}})) 
\end{math}
\vspace{0.5cm} \\
\begin{math}
m/M = \frac{l_{POST} \#r_{PRE}}{\#r_{PRE} l_{PRE}} 
- \frac{l_{POST} \#r_{POST} l_{PRE}}{\#r_{POST} l_{PRE} l_{POST}}
\end{math}
\vspace{0.5cm} \\
And finally the formula to obtain $m/M$ using only read counts and lengths:
\begin{math}
m/M = \frac{l_{POST} \#r_{PRE}}{l_{PRE} \#r_{POST}} -1
\end{math}
\vspace{0.5cm} \\
\textbf{XXX TODO check equations}

As a simple emblematic case suppose that the PRE and POST portion have the same lengths and that the
