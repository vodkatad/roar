...
\date{\Rpackage{roar} version \Sexpr{packageDescription("roar")$Version} (Last revision 2013-11-19)}
\begin{document}
...
in UCSC) and the more proximal  (with respect to the TSS, that is the farther to the canonical end
...
two (and only two) times in the gtf, one with the suffix ``\_PRE'' and one with ``\_POST''.
...
possible combinations p-values for multiple samples analysis. Single users will be able
to apply more significant procedures (eg. the Fisher or the Kost methods~\cite{Fisher,Kost})
to combine p-values considering their specific experimental setting.
