...
arguments <- matrix(c(
   ...
   't_samples_order', 'o', 1, "character",
   'c_samples_order', 'p', 1, "character"
), ncol=4, byrow=T)
...
if (is.null(opt$t_samples_order) | is.null(opt$c_samples_order)) {
   stop("Missing treatment or control samples order [-o, -p followed by comma separated bam files] param")
}
...
treatmentSamples <- as.vector(unlist(strsplit(opt$t_samples_order, ",")), mode="integer")
controlSamples <- as.vector(unlist(strsplit(opt$c_samples_order, ",")), mode="integer")
...
workOnChr <- function(chr) {
   ...
   # Fisher test
   rds <- computePairedPvals(rds, treatmentSamples, controlSamples)
